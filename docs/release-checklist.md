...
- [ ] Acceptance proves app-data, Credential Manager/Keychain, and installed-app paths were absent before mutation. Cleanup flags start false before those checks; app data is removed only after this run observes the newly created exact canonical `com.smartcat.translate` path, uninstall runs only for this run's install, and the macOS app copy is removed only when this run created it. CI markers never authorize deletion of pre-existing data.
