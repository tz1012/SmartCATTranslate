...
for(const [target,key,pattern] of [['x86_64-pc-windows-msvc','windows-x86_64',/\.nsis\.zip$/],['x86_64-apple-darwin','darwin-x86_64',/\.app\.tar\.gz$/],['aarch64-apple-darwin','darwin-aarch64',/\.app\.tar\.gz$/]]) { const bundle=files.find((file)=>file.includes(target)&&pattern.test(file)); if(!bundle) throw new Error(`updater_bundle_missing:${target}`); const signature=await readFile(`${bundle}.sig`,'utf8'); platforms[key]={signature:signature.trim(),url:`https://github.com/${repository}/releases/download/${tag}/${encodeURIComponent(basename(bundle))}`,size:(await stat(bundle)).size}; }
