...
if [[ "$mode" == --ci-ephemeral && "${CI:-}" != true ]]; then echo '--ci-ephemeral requires CI=true.' >&2; exit 1; fi
...
cleanup() { hdiutil detach "$mount" -quiet 2>/dev/null || true; if [[ "$mode" == --ci-ephemeral ]]; then case "$root" in "${TMPDIR:-/tmp}"/smartcat-release-acceptance.*) rm -rf -- "$root";; *) echo 'refusing unsafe cleanup' >&2; exit 1;; esac; fi; }
trap cleanup EXIT
...
if [[ "$mode" != --ci-ephemeral ]]; then echo "Dry acceptance passed; signed/notarized copy retained at $root and was not launched."; exit 0; fi
...
CI=true SMARTCAT_ACCEPTANCE_MODE=1 SMARTCAT_ACCEPTANCE_ROOT="$data" HOME="$data/home" TMPDIR="$data/tmp" "$executable" >"$root/app.log" 2>&1 &
pid=$!; deadline=$((SECONDS + 25))
while [[ ! -f "$data/app-ready.json" && $SECONDS -lt $deadline ]] && kill -0 "$pid" 2>/dev/null; do sleep 0.25; done
[[ -f "$data/app-ready.json" ]] || { echo 'App did not reach hydrated main-window readiness in time.' >&2; exit 1; }
kill "$pid" 2>/dev/null || true; wait "$pid" 2>/dev/null || true
echo 'CI ephemeral macOS signature, notarization, staple, isolated launch-ready assertions passed.'
