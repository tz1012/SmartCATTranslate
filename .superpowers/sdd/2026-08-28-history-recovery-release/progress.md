...
Tasks 5–7: publication fix round 2 — Windows feed must use Tauri v2 signed updater ZIPs rather than raw installers; production acceptance bypasses are forbidden; actual CI acceptance must exercise the OS key store/default app data; LKG needs successful async application-state hydration plus stability; SPDX/CycloneDX validation and manifest/checksum/attestation identity are publication gates. The same user ruling continues: no new negative/E2E/full regression tests are added, and this reduced proof depth remains explicit.

Tasks 5–7: publication fix round 3 — CI runner markers are not deletion authority. Acceptance cleanup uses false-by-default ownership flags established before preconditions, rejects existing app data/credential/install state, and removes only exact paths/install/copies actually created and observed by this run. No new failure tests were added per user instruction; positive script parsing/policy/records gates are the bounded evidence.
Tasks 5–7: packaging follow-up — Tauri packaging failed before Cargo because the updater verifier introduced a second binary and the package had no default. Cargo now selects `smartcat-translate` through `default-run` while preserving `verify-updater-key`; workflow policy makes both facts mandatory. The packaging agent must retry the actual package build.
